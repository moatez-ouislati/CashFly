...
class ElasticaToModelTransformerConfig 
{
    ...
    public function hints(array $value = []): \Symfony\Config\FosElastica\IndexesConfig\IndexPrototype\Persistence\ElasticaToModelTransformer\HintsConfig
    {
        $this->_usedProperties['hints'] = true;

        return $this->hints[] = new \Symfony\Config\FosElastica\IndexesConfig\IndexPrototype\Persistence\ElasticaToModelTransformer\HintsConfig($value);
    }
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('hints', $value)) {
            $this->_usedProperties['hints'] = true;
            $this->hints = array_map(fn ($v) => new \Symfony\Config\FosElastica\IndexesConfig\IndexPrototype\Persistence\ElasticaToModelTransformer\HintsConfig($v), $value['hints']);
            unset($value['hints']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['hints'])) {
            $output['hints'] = array_map(fn ($v) => $v->toArray(), $this->hints);
        }
