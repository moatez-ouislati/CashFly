...
export default class extends Controller {
    ...
    connect() {
        ...
        this.removeEventFromData = (id) => {
            Object.keys(this.CALENDAR_DATA).forEach(k => {
                if (Array.isArray(this.CALENDAR_DATA[k])) {
                    this.CALENDAR_DATA[k] = this.CALENDAR_DATA[k].filter(ev => Number(ev.id) !== Number(id));
                }
            });
        };

        this.refreshCalendarData = async () => {
            try {
                const resp = await fetch('/jpo/calendar-data');
