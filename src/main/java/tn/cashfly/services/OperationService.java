...
public class OperationService implements IOperationService {
    ...
    private void ensureSchema() {
        if (cnx == null) return;
        ensureColumnExists("OPÉRATIONS", "id_tresorerie", "ALTER TABLE OPÉRATIONS ADD COLUMN id_tresorerie INT NULL");
        ensureColumnExists("OPÉRATIONS", "reference", "ALTER TABLE OPÉRATIONS ADD COLUMN reference VARCHAR(64) NULL");
        ensureColumnExists("OPÉRATIONS", "facture", "ALTER TABLE OPÉRATIONS ADD COLUMN facture VARCHAR(64) NULL");
        ensureColumnExists("OPÉRATIONS", "pdf_url", "ALTER TABLE OPÉRATIONS ADD COLUMN pdf_url VARCHAR(255) NULL");
        ensureColumnExists("OPÉRATIONS", "type", "ALTER TABLE OPÉRATIONS ADD COLUMN type VARCHAR(20) NULL");
        ensureColumnExists("OPÉRATIONS", "categorie", "ALTER TABLE OPÉRATIONS ADD COLUMN categorie VARCHAR(128) NULL");
        ensureColumnExists("OPÉRATIONS", "description", "ALTER TABLE OPÉRATIONS ADD COLUMN description TEXT NULL");
        ensureColumnExists("OPÉRATIONS", "date_operation", "ALTER TABLE OPÉRATIONS ADD COLUMN date_operation TIMESTAMP NULL");
    }
    ...
    private void ensureColumnExists(String table, String column, String alterSql) {
        ...
        try (PreparedStatement ps = cnx.prepareStatement(checkSql)) {
            ...
            log.warn("Impossible de vérifier/ajouter la colonne {} dans {}", column, table, ignored);
        }
    }
    @Override
    ...
    private OPÉRATIONS mapRowToOperation(ResultSet rs) throws SQLException {
        int idTresorerie = 0;
        try {
            idTresorerie = rs.getInt("id_tresorerie");
        } catch (SQLException ignored) {
        }
        TRÉSORERIE tresorerie = null;
        if (idTresorerie > 0) {
            tresorerie = tresorerieService.getById(idTresorerie);
        }
    ...
    private OPÉRATIONS mapRowToOperation(ResultSet rs, TRÉSORERIE tresorerie) throws SQLException {
        ...
        if (typeStr == null || typeStr.isBlank()) {
            try {
                String legacy = rs.getString("type_operation");
                if (legacy != null) {
                    if ("RECETTE".equalsIgnoreCase(legacy)) typeStr = "revenu";
                    else if ("DEPENSE".equalsIgnoreCase(legacy)) typeStr = "depense";
                }
            } catch (SQLException ignored) {
            }
        }
        OPÉRATIONS.TypeOperation type = null;
        if (typeStr != null && !typeStr.isBlank()) {
            if ("revenu".equalsIgnoreCase(typeStr)) type = OPÉRATIONS.TypeOperation.revenu;
            else if ("depense".equalsIgnoreCase(typeStr)) type = OPÉRATIONS.TypeOperation.depense;
        }
