...
class JpoController extends AbstractController
{
    ...
    public function myEvents(Request $request, PaginatorInterface $paginator): Response
    {
        ...
        $sortBy  = $request->query->get('sortBy', 'dateEvenement');
    ...
    public function eventsOnDay(Request $request): JsonResponse
    {
        ...
        $data = array_map(fn($e) => [
            'id'               => $e->getIdEvenement(),
            'titre'            => $e->getTitre(),
            'date'             => $e->getDateEvenement()->format('Y-m-d'),
            'lieu'             => $e->getLieu(),
            'id_createur'      => $e->getIdCreateur(),
            'max_participants' => $e->getMaxParticipants(),
            'current_participants' => $e->getCurrentParticipants(),
    ...
    #[Route('/calendar-data', name: 'calendar_data', methods: ['GET'])]
    public function apiCalendarData(): JsonResponse
    {
        $allEvents = $this->jpoRepository->findAll();
        $calendarData = [];
        foreach ($allEvents as $e) {
            $key = $e->getDateEvenement()->format('Y-m-d');
            $calendarData[$key][] = [
                'id'                   => $e->getIdEvenement(),
                'titre'                => $e->getTitre(),
                'lieu'                  => $e->getLieu(),
    ...
    public function register(int $id, EntityManagerInterface $em): JsonResponse
    {
        ...
        // Mock AI Suggestion (Service currently unavailable)
        $suggestionData = null;
    ...
    /**
     * API: Join the waiting list for an event (investisseur)
     */
    #[Route('/join-waitlist/{id}', name: 'join_waitlist', methods: ['POST'])]
    #[IsGranted('ROLE_INVESTISSEUR')]
    public function joinWaitlist(int $id, EntityManagerInterface $em, ListeAttenteRepository $waitlistRepo): JsonResponse
    {
        /** @var User|null $user */
        $user = $this->getUser();
        if (!$user) {
        ...
        if (!$event->isFull()) {
            return new JsonResponse(['error' => 'L\'événement n\'est pas encore complet. Vous pouvez vous inscrire directement.'], 400);
        }
        ...
        $existing = $waitlistRepo->findOneBy([
            'idEvenement' => $id,
            'idUtilisateur' => $user->getId()
        ]);

        if ($existing) {
            return new JsonResponse(['error' => 'Vous êtes déjà sur la liste d\'attente'], 400);
        }

        // Determine position
        $lastEntry = $waitlistRepo->findOneBy(['idEvenement' => $id], ['position' => 'DESC']);
        $newPosition = $lastEntry ? $lastEntry->getPosition() + 1 : 1;

        $waitEntry = new ListeAttente();
        $waitEntry->setIdEvenement($id);
        $waitEntry->setIdUtilisateur($user->getId());
        $waitEntry->setPosition($newPosition);
        $waitEntry->setDateDemande(new \DateTime());

        $em->persist($waitEntry);
        $em->flush();
    ...
    public function createEvent(Request $request): JsonResponse
    {
        ...
        return new JsonResponse([
            ...
            'id_createur'      => $event->getIdCreateur(),
            'date'             => $event->getDateEvenement()->format('Y-m-d'),
        ]);
    }
    ...
    public function editEvent(int $id, Request $request): JsonResponse
    {
        ...
        if ($dateStr) {
            try {
                ...
                $event->setDateEvenement($newDate);
            } catch (\Exception $e) {
                // Ignore invalid date or log it
            }
        }
    ...
    public function eventDetails(int $id): Response
    {
        ...
        if ($user) {
            ...
            // Check Waitlist
            $waitEntry = $em->getRepository(ListeAttente::class)->findOneBy([
                'idEvenement' => $event->getIdEvenement(),
                'idUtilisateur' => $user->getId()
            ]);
            if ($waitEntry) {
        ...
        // Waitlist count
        $waitlistCount = $em->getRepository(ListeAttente::class)->count(['idEvenement' => $event->getIdEvenement()]);
    ...
    #[IsGranted('ROLE_PROPRIETAIRE')]
    public function exportExcel(int $id, ParticipationJpoRepository $partRepo, UserRepository $userRepo): Response
    {
        /** @var Utilisateur|null $user */
        $user = $this->getUser();
        $event = $this->jpoRepository->find($id);
        ...
        foreach ($participants as $p) {
            ...
            if ($participantUser) {
                $sheet->setCellValue('A' . $row, trim($participantUser->getNom() . ' ' . $participantUser->getPrenom()) ?: 'Non renseigné');
    ...
    public function unregister(int $id, EntityManagerInterface $em): JsonResponse
    {
        ...
        try {
            ...
            // Check for auto-promotion from waitlist
            $waitlistRepo = $em->getRepository(ListeAttente::class);
            $nextInLine = $waitlistRepo->findOneBy(
                ['idEvenement' => $event->getIdEvenement()],
                ['position' => 'ASC']
            );
            ...
            if ($nextInLine) {
                // Promote this user
                $newParticipation = new ParticipationJpo();
                $newParticipation->setIdEvenement($event->getIdEvenement());
                $newParticipation->setIdUtilisateur($nextInLine->getIdUtilisateur());
                $newParticipation->setStatut('confirmé');
                $newParticipation->setDateInscription(new \DateTime());
                $newParticipation->setBadgeGenere(false);

                $em->persist($newParticipation);
                $em->remove($nextInLine);

                // Re-order remaining waitlist (optional but good practice)
                $remaining = $waitlistRepo->findBy(
                    ['idEvenement' => $event->getIdEvenement()],
                    ['position' => 'ASC']
                );
                $pos = 1;
                foreach ($remaining as $item) {
                    if ($item->getIdAttente() !== $nextInLine->getIdAttente()) {
                        $item->setPosition($pos++);
                    }
                }
                
                // Note: currentParticipants doesn't change because one leaves and one enters
            } else {
            ...
            return new JsonResponse(['error' => 'Erreur lors de la désinscription'], 500);
        }
    }
    ...
    public function generateBadge(int $id, EntityManagerInterface $em): Response
    {
        ...
        // User Info
        $userName = mb_substr(trim($user->getNom() . ' ' . $user->getPrenom()) ?: $user->getEmail(), 0, 30);
        ...
        $fileName = 'Badge_' . preg_replace('/[^a-zA-Z0-9_\-]/', '_', $event->getTitre()) . '_' . preg_replace('/[^a-zA-Z0-9_\-]/', '_', trim($user->getNom() . ' ' . $user->getPrenom()) ?: 'User') . '.jpg';
    ...
    public function verifyBadge(int $eventId, int $investorId, EntityManagerInterface $em, UserRepository $userRepo): Response
    {
        ...
        return $this->render('jpo/badge_scan_result.html.twig', [
            'status' => $status,
            'nomComplet' => $user ? (trim($user->getNom() . ' ' . $user->getPrenom()) ?: $user->getEmail()) : 'Inconnu',
