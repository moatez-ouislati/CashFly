...
class ChatController extends AbstractController
{
    ...
    private function canAccessChat(JourneePorteOuverte $event, User $user): bool
    {
    ...
    public function loadMessages(int $eventId): JsonResponse
    {
        ...
        foreach ($messages as $msg) {
            $author = $this->UserRepository->find($msg->getIdUtilisateur());
            $isAdmin = in_array('ROLE_ADMIN', $author->getRoles(), true);
            $authorData = $author ? [
                'id' => $author->getId(),
                'nom' => $isAdmin ? 'Administrateur' : trim($author->getNom() . ' ' . $author->getPrenom()),
            ...
            if ($msg->getReplyTo()) {
                ...
                if ($parent) {
                    $parentAuthor = $this->UserRepository->find($parent->getIdUtilisateur());
                    $replyData = [
                        'author' => $parentAuthor ? trim($parentAuthor->getNom() . ' ' . $parentAuthor->getPrenom()) : 'Inconnu',
