...
class ChatbotController extends AbstractController
{
    ...
    {
        if (!$this->isGranted('ROLE_PROPRIETAIRE') && !$this->isGranted('ROLE_INVESTISSEUR')) {
            throw $this->createAccessDeniedException('Accès réservé aux propriétaires et investisseurs.');
        }
    ...
    public function sendMessage(
        ...
        $user = $this->getUser();
        $isInvestor = $this->isGranted('ROLE_INVESTISSEUR');
        $role = $isInvestor ? 'investisseur' : 'proprietaire';
        
        $context = $isInvestor 
            ? $this->buildInvestorContext($user, $investmentService)
            : $this->buildFinancialContext($user, $tresorerieRepository);

        $response = $chatbotService->sendMessage($message, $context, $role);
    ...
    private function buildInvestorContext($user, InvestmentService $investmentService): array
    {
        $stats = $investmentService->getPortfolioStats($user);
        return [
            'stats' => $stats,
            'user_budget' => (float)$user->getBudget(),
            'user_experience' => $user->getYearsExperience()
        ];
    }
}
