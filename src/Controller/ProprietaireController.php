...
/**
 * Controller managing the Proprietaire (Business Owner) dashboard and profile.
 * Restricted to users with the ROLE_PROPRIETAIRE role.
 */
#[Route('/proprietaire')]
...
class ProprietaireController extends AbstractController
{
    ...
    /**
     * Renders the main dashboard for the Proprietaire.
     */
    ...
    #[Route('/investments', name: 'proprietaire_investments')]
    public function investments(EntityManagerInterface $em): Response
    {
        /** @var User $user */
        $user = $this->getUser();
        
        // Récupérer les investissements reçus pour les entreprises du propriétaire
        $investments = $em->getRepository(Investissement::class)->createQueryBuilder('i')
            ->join('i.entreprise', 'e')
        ...
        return $this->render('proprietaire/investments.html.twig', [
            'investments' => $investments,
        ]);
    }

    #[Route('/investment/approve/{id}', name: 'proprietaire_investment_approve', methods: ['POST'])]
    public function approveInvestment(int $id): Response
    {
        if ($this->investmentService->approveInvestissement($id)) {
            $this->addFlash('success', 'Investissement approuvé avec succès.');
        } else {
            $this->addFlash('error', "Impossible d'approuver l'investissement.");
        }

        return $this->redirectToRoute('proprietaire_investments');
    }

    #[Route('/investment/reject/{id}', name: 'proprietaire_investment_reject', methods: ['POST'])]
    public function rejectInvestment(int $id): Response
    {
        if ($this->investmentService->rejectInvestissement($id)) {
            $this->addFlash('success', 'Investissement rejeté.');
        } else {
            $this->addFlash('error', "Impossible de rejeter l'investissement.");
        }
...
$mailer->send($email);
    ...
    /**
     * Lists all investors available in the platform.
     * Supports AJAX requests for dynamic searching.
     */
    ...
    public function clients(Request $request, EntityManagerInterface $em): Response
    {
        ...
        $qb = $em->getRepository(User::class)->createQueryBuilder('u')
            ->where('u.roles = :role')
            ->setParameter('role', 'investisseur');
