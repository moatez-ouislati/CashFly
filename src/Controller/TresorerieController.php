...
class TresorerieController extends AbstractController
{
    ...
    private function calculateCashFlowForecast(array $tresoreries, \App\Repository\OperationRepository $operationRepository): array
    {
        ...
        foreach ($tresoreries as $tresorerie) {
            ...
            if (count($operations) === 0) {
                // If no operations, balance remains constant
                for ($day = 1; $day <= 30; $day++) {
                    $globalProjections[$day]['balance'] += $currentBalance;
                }
                continue;
            }
            ...
            for ($day = 1; $day <= 30; $day++) {
                ...
                $globalProjections[$day]['balance'] += $projectedBalance;
                $globalProjections[$day]['expected_revenu'] += $avgDailyRevenu;
                $globalProjections[$day]['expected_depense'] += $avgDailyDepense;
        ...
        return [
            ...
            'accounts' => $forecast,
            'global_projections' => array_values($globalProjections),
            'alerts' => $alerts,
     ...
     */
    private function checkTresorerieAccess(Tresorerie $tresorerie, bool $isViewOnly = false): void
    {
        $user = $this->getUser();
        ...
        if ($tresorerie->getEntreprise()->getProprietaire() !== $user) {
    ...
    {
        $this->checkTresorerieAccess($tresorerie, true);
