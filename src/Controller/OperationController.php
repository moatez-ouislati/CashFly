...
class OperationController extends AbstractController
{
    ...
    public function adminIndex(Request $request, OperationRepository $operationRepository): Response
    {
        ...
        $qb = $operationRepository->createQueryBuilder('o')
            ->addSelect('t', 'e', 'p') // Optimisation DoctrineDoctor : Eager Loading
            ->join('o.tresorerie', 't')
            ->join('t.entreprise', 'e')
    ...
    public function new(Request $request, EntityManagerInterface $entityManager, OperationRepository $operationRepository, CurrencyConverterService $currencyConverterService): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $entityManager->flush(); // flush operation first to get its ID if it's new
            
            // Handle notesInterne
            $notesContent = $form->get('notesInterne')->getData();
            if (!empty($notesContent)) {
                $note = new \App\Entity\OperationNote();
                $note->setIdOperation($operation->getId());
            ...
            if ((float)$tresorerie->getSolde() < 100) {
                $this->addFlash('warning', 'Attention : Le solde du compte ' . $tresorerie->getNomCompte() . ' est très bas (' . number_format((float)$tresorerie->getSolde(), 2) . ' TND).');
            }
     ...
     */
    private function checkOperationAccess(Operation $operation, bool $isViewOnly = false): void
    {
        $user = $this->getUser();
        ...
        if ($operation->getTresorerie()->getEntreprise()->getProprietaire() !== $user) {
    ...
    {
        $this->checkOperationAccess($operation, true);
    ...
    public function edit(Request $request, Operation $operation, EntityManagerInterface $entityManager, \App\Service\CurrencyConverterService $currencyConverterService): Response
    {
        ...
        $existingNote = $entityManager->getRepository(\App\Entity\OperationNote::class)->findOneBy(['idOperation' => $operation->getId()]);
        if ($existingNote) {
            $form->get('notesInterne')->setData($existingNote->getContent());
        }
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // Handle notesInterne update
            $notesContent = $form->get('notesInterne')->getData();
            $note = $entityManager->getRepository(\App\Entity\OperationNote::class)->findOneBy(['idOperation' => $operation->getId()]);
            ...
            if (!empty($notesContent)) {
                ...
                // If note content was cleared in the form, remove the note entity
    ...
    public function apiRecent(OperationRepository $operationRepository): JsonResponse
    {
        ...
        $operations = $qb->getQuery()->getArrayResult();
